...
public class InMemoryCartRepository implements CartRepository{
	...
	public Cart create(Cart cart) {
		if( listOfCarts.keySet().contains( cart.getCartId() ) ){
			throw new IllegalArgumentException( String.format( "Can not create"
						+ "a cart. A cart with the give id (%) aldrady exist", cart.getCartId() ) );
		}
	...
	public void update(String cartId, Cart cart) {
		if( !listOfCarts.keySet().contains(cartId) ){
			...
			+ " the cart. The cart with the give id (%) does not exist", cartId) );
		}
	...
	public void delete(String cartId) {
		if(!listOfCarts.keySet().contains(cartId)) {
			...
			+ "the cart. The cart with the give id (%) does not does not exist", cartId) );
		}
