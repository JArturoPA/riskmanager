...
public class ProductController {
	...
	public String filterProducts(@PathVariable("category") String productCategory,
					...
					Model model){
		model.addAttribute("product", productService.getProductsByManufacturer(productCategory));
	...
	public String processAddNewProductForm(@ModelAttribute("newProduct") @Valid Product newProduct, BindingResult result,
		...
		if( suppressedFields.length > 0 ){
			throw new RuntimeException("Atempting to bind disallowed fields: " 
		...
		if(productImage != null && !productImage.isEmpty()){
			try{
				...
				throw new RuntimeException("Product Image or PDF saving failed", e);
			}
		}
